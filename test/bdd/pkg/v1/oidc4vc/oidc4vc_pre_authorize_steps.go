...
func (s *PreAuthorizeStep) RegisterSteps(sc *godog.ScenarioContext) {
	sc.Step(`^issuer with id "([^"]*)" wants to issue credentials to his client$`, s.prepareIssuer)
	sc.Step(`^issuer sends request to initiate-issuance with pre-auth code flow$`, s.initiateIssuance)

	sc.Step(`^issuer receives response he shows qr code with redirect urls$`, s.parseUrl)
	sc.Step(`^client should scan code and be redirected to pre-authorize code flow page$`, s.parseUrl)
	sc.Step(`^client receives authorization token$`, s.receiveToken)
}
